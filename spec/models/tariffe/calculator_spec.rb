...
describe Tariffe::Calculator do
  context 'with defaults' do
    ...
    it { expect(subject.params_order).to eq({ tariffe_calculator: { sort_order: 'desc' } }.with_indifferent_access) }
    it { expect(subject.filled?).to be_false }
  end

  context 'with data' do
    subject { Tariffe::Calculator.new(amount_of_invoices: 10, household_fee: 100.0, sort_order: 'desc') }

    it { expect(subject.sort_order).to eq('desc') }
    it { expect(subject.reverse_order).to eq('asc') }
    it { expect(subject.asc?).to be_false }
    it { expect(subject.params_order).to eq({ tariffe_calculator: { sort_order: 'asc' } }.with_indifferent_access) }
